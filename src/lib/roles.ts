...
export type Area='Home'|'LeadGeneration'|'ColdCalling'|'Content'|'Campaigns'|'Approvals'|'Calendar'|'MeetingsNotes'|'People'|'Integrations'|'AdminAddTab'|'Inbox'|'Tasks'|'Attendance'|'Pipelines'|'MyTasks'
...
export const can=(role:Role,area:Area,need:Permission)=>{const p=permissionMatrix[area]?.[role]||'None';if(need==='Read')return p!=='None';if(need==='CheckOnly')return p==='CheckOnly'||p==='CRUD';if(need==='CRUD')return p==='CRUD';return false}
